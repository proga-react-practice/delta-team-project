...
const Form: React.FC<FormProps> = ({ onSubmit }) => {
  ...
  return (
    <Box component="form"  onSubmit={handleSubmit(onSubmit)} sx={BoxFormStyle}>
      ...
      <TextField
        label="Model"
        {...register("model", { required: "Model is required", validate: {
          isFirstLetterUppercase: value =>
            /^[A-Z]/.test(value) || "The first letter must be uppercase"
        }})}
        sx={TextFieldStyle}
      ...
      <TextField
        label="Year"
        type="number"
        {...register("year", { required: "Year is required",  min: { value: 1900, message: "Year must be 1900 or later" },
        max: { value: new Date().getFullYear(), message: "Year must be this year or earlier" } })}
        sx={TextFieldStyle}
        error={Boolean(errors.year)}
        helperText={errors.year?.message}
      />
      <FormControl   error={Boolean(errors.body_type)}
  sx={TextFieldStyle} >
  ...
  {errors.body_type && <FormHelperText error>{errors.body_type.message}</FormHelperText>}
      ...
      <TextField
        label="Mileage"
        type="number"
        {...register("mileage_km", { required: "Mileage is required",     min: { value: 0, message: "Mileage must be 0 or bigger" },
...
/>
      ...
      <TextField
        label="Price Per Day"
        type="number"
        {...register("price_per_day", { required: "Price is required",  min: { value: 1, message: "Price must be 1 or bigger" }})}
      ...
      <TextField
        label="Horse Power"
        type="number"
        {...register("horse_power", { required: "Horse power is required",     min: { value: 1, message: "Horse power must be 1 or bigger" },
      ...
      <TextField
        ...
        type="number"
        {...register("engine_capacity", { required: "Engine capacity is required", min: { value: 0.1, message: "Engine capacity must be 0.1 or bigger" },
